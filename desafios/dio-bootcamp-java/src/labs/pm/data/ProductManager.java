...
public class ProductManager {
	...
	public Product reviewProduct(Product product, Rating rating, String comments) {
		...
		for (; i < reviews.length;) {
			if (reviews[i] == null) {
				reviews[i] = new Review(rating, comments);
				break;
			}
			stars += reviews[i].getRating().ordinal();
			i++;
		}

		this.product = product.applyRating(Math.round((float) stars / i));
		return this.product;
	}
	...
	public void printProductReport() {
		...
		txt.append(MessageFormat.format(resource.getString("product"), product.getName(),
		...
		for (Review review : reviews) {
			if (review == null)
				continue;
			txt.append(MessageFormat.format(resource.getString("review"), review.getRating().getStars(),
					review.getComments()));
			txt.append("\n");
		}
